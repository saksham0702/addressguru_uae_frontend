// pages/blogs/[slug].js

import { useCallback } from "react";
import { useRouter } from "next/router";
import Head from "next/head";
import Link from "next/link";
import Image from "next/image";
import {
  getBlogDetails,
  getCategories,
  getMostViewedBlogs,
} from "@/api/uae-blogs";
import { FaTwitter, FaLinkedin, FaGithub, FaGlobe } from "react-icons/fa";

const APP_URL = "https://addressguru.ae/api";

// ─── SSR ────────────────────────────────────────────────────────────────────
export async function getServerSideProps({ params, req }) {
  const { slug } = params;

  try {
    const [blogRes, categoriesRes, mostViewedRes] = await Promise.all([
      getBlogDetails(slug),
      getCategories(),
      getMostViewedBlogs(),
    ]);

    if (!blogRes?.status || !blogRes?.data) {
      return { notFound: true }; // renders Next.js 404 page
    }

    // Build canonical URL from request headers
    const protocol = req.headers["x-forwarded-proto"] || "https";
    const host = req.headers["x-forwarded-host"] || req.headers.host;
    const canonicalUrl = `${protocol}://${host}/blogs/${slug}`;

    return {
      props: {
        blogDetail: blogRes.data,
        blogCategories: categoriesRes?.data ?? [],
        mostViewedBlogs: mostViewedRes?.data?.blogs ?? [],
        canonicalUrl,
      },
    };
  } catch (err) {
    console.error("SSR error:", err);
    return { notFound: true };
  }
}

// ─── Helpers ────────────────────────────────────────────────────────────────
function getIcon(name) {
  switch (name.toLowerCase()) {
    case "twitter":
      return <FaTwitter />;
    case "linkedin":
      return <FaLinkedin />;
    case "github":
      return <FaGithub />;
    default:
      return <FaGlobe />;
  }
}

function cleanHtml(html) {
  if (!html) return "";
  return html
    .replace(/<\/?(div|section|article)[^>]*>/gi, "")
    .replace(/class="[^"]*"/gi, "")
    .replace(/style="[^"]*"/gi, "")
    .replace(/<img[^>]*src="data:image[^"]*"[^>]*>/gi, "")
    .replace(/<h[1-6][^>]*>\s*<img[^>]*>\s*<\/h[1-6]>/gi, "")
    .replace(/<p>\s*<\/p>/gi, "");
}

// ─── Component ───────────────────────────────────────────────────────────────
export default function BlogDetail({
  blogDetail,
  blogCategories,
  mostViewedBlogs,
  canonicalUrl,
}) {
  const router = useRouter();

  const handleShare = useCallback(
    (platform) => {
      const url = encodeURIComponent(canonicalUrl);
      const text = encodeURIComponent(blogDetail?.title || "");
      const shareUrls = {
        facebook: `https://www.facebook.com/sharer/sharer.php?u=${url}`,
        twitter: `https://twitter.com/intent/tweet?url=${url}&text=${text}`,
        linkedin: `https://www.linkedin.com/sharing/share-offsite/?url=${url}`,
      };
      if (shareUrls[platform]) {
        window.open(shareUrls[platform], "_blank", "width=600,height=400");
      }
    },
    [blogDetail?.title, canonicalUrl],
  );

  // ── SEO values ──────────────────────────────────────────────────────────
  const seoTitle = blogDetail?.seo?.title || blogDetail?.title || "Blog";
  const seoDescription =
    blogDetail?.seo?.description || blogDetail?.excerpt || "";
  const seoKeywords = blogDetail?.seo?.keywords || "";
  const ogImage = blogDetail?.seo?.ogImage
    ? `${APP_URL}/${blogDetail.seo.ogImage}`
    : `${APP_URL}/${blogDetail?.coverImage}`;

  // Structured data (JSON-LD) for Google
  const jsonLd = {
    "@context": "https://schema.org",
    "@type": "BlogPosting",
    headline: blogDetail?.title,
    description: seoDescription,
    image: ogImage,
    datePublished: blogDetail?.publishedAt || blogDetail?.createdAt,
    dateModified: blogDetail?.updatedAt || blogDetail?.createdAt,
    author: {
      "@type": "Person",
      name: blogDetail?.author?.name || "Unknown",
    },
    publisher: {
      "@type": "Organization",
      name: "AddressGuru UAE",
      logo: {
        "@type": "ImageObject",
        url: "https://addressguru.ae/logo.png",
      },
    },
    mainEntityOfPage: {
      "@type": "WebPage",
      "@id": canonicalUrl,
    },
    ...(blogDetail?.readingTime && {
      timeRequired: `PT${blogDetail.readingTime}M`,
    }),
  };

  // Breadcrumb structured data
  const breadcrumbJsonLd = {
    "@context": "https://schema.org",
    "@type": "BreadcrumbList",
    itemListElement: [
      {
        "@type": "ListItem",
        position: 1,
        name: "Home",
        item: "https://addressguru.ae",
      },
      {
        "@type": "ListItem",
        position: 2,
        name: "Blogs",
        item: "https://addressguru.ae/blogs",
      },
      {
        "@type": "ListItem",
        position: 3,
        name: blogDetail?.category_id?.name || "Category",
        item: `https://addressguru.ae/blogs/category/${blogDetail?.category_id?.slug}`,
      },
      {
        "@type": "ListItem",
        position: 4,
        name: blogDetail?.title,
        item: canonicalUrl,
      },
    ],
  };

  // FAQ structured data
  let faqJsonLd = null;
  try {
    const faqs =
      typeof blogDetail?.faqs === "string"
        ? JSON.parse(blogDetail.faqs)
        : blogDetail?.faqs;
    if (faqs?.length > 0) {
      faqJsonLd = {
        "@context": "https://schema.org",
        "@type": "FAQPage",
        mainEntity: faqs.map((f) => ({
          "@type": "Question",
          name: f.question,
          acceptedAnswer: { "@type": "Answer", text: f.answer },
        })),
      };
    }
  } catch (_) {}

  return (
    <>
      <Head>
        {/* ── Primary ── */}
        <title>{seoTitle}</title>
        <meta name="description" content={seoDescription} />
        {seoKeywords && <meta name="keywords" content={seoKeywords} />}
        <meta name="robots" content="index, follow" />
        <link rel="canonical" href={canonicalUrl} />

        {/* ── Open Graph ── */}
        <meta property="og:type" content="article" />
        <meta property="og:title" content={seoTitle} />
        <meta property="og:description" content={seoDescription} />
        <meta property="og:image" content={ogImage} />
        <meta property="og:image:width" content="1200" />
        <meta property="og:image:height" content="630" />
        <meta property="og:url" content={canonicalUrl} />
        <meta property="og:site_name" content="AddressGuru UAE" />
        <meta
          property="article:published_time"
          content={blogDetail?.publishedAt}
        />
        <meta
          property="article:modified_time"
          content={blogDetail?.updatedAt}
        />
        <meta property="article:author" content={blogDetail?.author?.name} />
        <meta
          property="article:section"
          content={blogDetail?.category_id?.name}
        />

        {/* ── Twitter Card ── */}
        <meta name="twitter:card" content="summary_large_image" />
        <meta name="twitter:title" content={seoTitle} />
        <meta name="twitter:description" content={seoDescription} />
        <meta name="twitter:image" content={ogImage} />

        {/* ── JSON-LD Structured Data ── */}
        <script
          type="application/ld+json"
          dangerouslySetInnerHTML={{ __html: JSON.stringify(jsonLd) }}
        />
        <script
          type="application/ld+json"
          dangerouslySetInnerHTML={{ __html: JSON.stringify(breadcrumbJsonLd) }}
        />
        {faqJsonLd && (
          <script
            type="application/ld+json"
            dangerouslySetInnerHTML={{ __html: JSON.stringify(faqJsonLd) }}
          />
        )}
      </Head>

      {/* ── Page ── */}
      <div className="min-h-screen bg-white w-full md:w-[90%] max-w-[1800px] 2xl:w-[80%] px-4 mx-auto py-8">
        <div className="flex flex-col lg:flex-row gap-8">
          {/* ── Left Column ── */}
          <div className="lg:w-[calc(100%-396px)] min-w-0">
            {/* Breadcrumb */}
            <nav
              aria-label="breadcrumb"
              className="mb-2 flex items-center gap-2 text-sm text-gray-600"
            >
              <Link href="/" className="hover:text-[#FF6E04] transition-colors">
                Home
              </Link>
              <span>/</span>
              <Link
                href="/blogs"
                className="hover:text-[#FF6E04] transition-colors"
              >
                Blogs
              </Link>
              <span>/</span>
              <span className="text-gray-800 font-medium">
                {blogDetail?.category_id?.name}
              </span>
            </nav>

            {/* Header */}
            <div className="mb-4 text-left">
              <h1 className="2xl:text-2xl text-lg font-semibold text-gray-800 mb-2 leading-tight">
                {blogDetail?.title}
              </h1>
              <div className="flex flex-wrap items-center gap-4 text-sm text-gray-600">
                <p className="font-semibold text-gray-900 text-base leading-tight">
                  {blogDetail?.author?.name}
                </p>
                <div className="flex items-center gap-2">
                  <svg
                    className="w-5 h-5"
                    fill="currentColor"
                    viewBox="0 0 20 20"
                  >
                    <path
                      fillRule="evenodd"
                      d="M6 2a1 1 0 00-1 1v1H4a2 2 0 00-2 2v10a2 2 0 002 2h12a2 2 0 002-2V6a2 2 0 00-2-2h-1V3a1 1 0 10-2 0v1H7V3a1 1 0 00-1-1zm0 5a1 1 0 000 2h8a1 1 0 100-2H6z"
                      clipRule="evenodd"
                    />
                  </svg>
                  <time dateTime={blogDetail?.publishedAt}>
                    {blogDetail?.date}
                  </time>
                </div>
                {blogDetail?.readingTime && (
                  <span>{blogDetail.readingTime} min read</span>
                )}
              </div>
            </div>

            {/* Cover Image */}
            <div
              className="mb-8 rounded-lg overflow-hidden shadow-lg w-full"
              style={{ height: "400px" }}
            >
              <Image
                src={`${APP_URL}/${blogDetail?.coverImage}`}
                alt={blogDetail?.title}
                className="h-full w-full object-cover"
                width={1200}
                height={400}
                quality={100}
                priority
                unoptimized
              />
            </div>

            {/* Share Buttons */}
            <div className="mb-8 flex items-center gap-4 pb-6 border-b text-sm border-gray-200">
              <span className="text-gray-600 font-medium">Share:</span>
              {[
                {
                  platform: "facebook",
                  label: "Facebook",
                  path: "M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z",
                },
                {
                  platform: "twitter",
                  label: "X (Twitter)",
                  path: "M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z",
                },
                {
                  platform: "linkedin",
                  label: "LinkedIn",
                  path: "M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z",
                },
              ].map(({ platform, label, path }) => (
                <button
                  key={platform}
                  onClick={() => handleShare(platform)}
                  className="flex items-center gap-2 text-gray-600 hover:text-[#FF6E04] transition-colors"
                  aria-label={`Share on ${label}`}
                >
                  <svg
                    className="w-5 h-5"
                    fill="currentColor"
                    viewBox="0 0 24 24"
                  >
                    <path d={path} />
                  </svg>
                  {label}
                </button>
              ))}
            </div>

            {/* Blog Content */}
            <article
              className="blog-content mb-12"
              dangerouslySetInnerHTML={{
                __html: cleanHtml(blogDetail?.content),
              }}
            />

            {/* FAQ */}
            {(() => {
              try {
                const faqs =
                  typeof blogDetail?.faqs === "string"
                    ? JSON.parse(blogDetail.faqs)
                    : blogDetail?.faqs;
                if (!faqs?.length) return null;
                return (
                  <section className="mb-12">
                    <h2 className="text-xl font-bold text-gray-800 mb-6 text-left">
                      Frequently Asked Questions
                    </h2>
                    <div className="w-16 h-1 bg-[#FF6E04] mb-6" />
                    <div className="space-y-4">
                      {faqs.map((faq, i) => (
                        <details
                          key={i}
                          className="group border border-gray-200 rounded-lg overflow-hidden"
                        >
                          <summary className="flex justify-between items-center p-4 cursor-pointer bg-gray-50 hover:bg-orange-50 transition-colors list-none">
                            <span className="font-semibold text-gray-800 pr-4 text-left">
                              {faq.question}
                            </span>
                            <span className="text-[#FF6E04] font-bold text-xl flex-shrink-0 group-open:rotate-45 transition-transform duration-200">
                              +
                            </span>
                          </summary>
                          <div className="p-4 text-gray-600 leading-relaxed border-t border-gray-100 bg-white text-left">
                            {faq.answer}
                          </div>
                        </details>
                      ))}
                    </div>
                  </section>
                );
              } catch {
                return null;
              }
            })()}

            {/* Author */}
            <div className="bg-white rounded-lg shadow-md border border-gray-100 p-5 mb-6">
              <h3 className="text-xl font-bold text-gray-800 mb-1">
                Author Profile
              </h3>
              <div className="w-16 h-1 bg-[#FF6E04] mb-5" />
              <div className="flex items-center gap-4 mb-4">
                {blogDetail?.author?.avatar ? (
                  <Image
                    width={64}
                    height={64}
                    className="w-16 h-16 rounded-full object-cover flex-shrink-0 border-2 border-[#FF6E04]/20"
                    src={`${APP_URL}/${blogDetail.author.avatar}`}
                    alt={blogDetail.author.name}
                  />
                ) : (
                  <div className="w-16 h-16 rounded-full bg-[#FF6E04]/10 flex items-center justify-center flex-shrink-0 text-2xl font-bold text-[#FF6E04]">
                    {blogDetail?.author?.name?.[0] ?? "A"}
                  </div>
                )}
                <div>
                  <p className="font-semibold text-gray-900 text-base leading-tight">
                    {blogDetail?.author?.name}
                  </p>
                  <p className="text-sm text-[#FF6E04] font-medium mt-0.5">
                    {blogDetail?.author?.JobTitle}
                  </p>
                </div>
              </div>
              {blogDetail?.author?.bio && (
                <p className="text-sm text-gray-500 leading-relaxed border-t border-gray-100 pt-4">
                  {blogDetail.author.bio}
                </p>
              )}
              <div className="flex items-center gap-3 text-xl mt-3">
                {Object.entries(blogDetail?.author?.social || {})
                  .filter(([, url]) => url)
                  .map(([platform, url]) => (
                    <a
                      key={platform}
                      href={url}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="text-gray-600 hover:text-blue-500 transition flex items-center gap-1"
                    >
                      {getIcon(platform)}
                      <span className="text-sm">{platform}</span>
                    </a>
                  ))}
              </div>
            </div>

            {/* Related Articles */}
            <section>
              <h2 className="text-xl font-bold text-gray-800 mb-6 text-left">
                Related Articles
              </h2>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {mostViewedBlogs?.slice(0, 3)?.map((blog) => (
                  <Link
                    key={blog.id}
                    href={`/blogs/${blog.slug}`}
                    className="bg-white rounded-lg overflow-hidden shadow-md hover:shadow-xl transition-shadow duration-300 border border-gray-100"
                  >
                    <div className="relative h-48 overflow-hidden">
                      <Image
                        src={`${APP_URL}/${blog.coverImage}`}
                        alt={blog.title}
                        width={500}
                        height={500}
                        quality={100}
                        className="w-full h-full object-cover hover:scale-110 transition-transform duration-500"
                      />
                      <div className="absolute top-3 left-3 bg-[#FF6E04] text-white px-3 py-1 rounded-full text-xs font-medium">
                        {blog?.category_id?.name}
                      </div>
                    </div>
                    <div className="p-4">
                      <h3 className="text-base font-bold text-gray-800 hover:text-[#FF6E04] transition-colors line-clamp-2 text-left">
                        {blog?.title}
                      </h3>
                    </div>
                  </Link>
                ))}
              </div>
            </section>
          </div>

          {/* ── Sidebar ── */}
          <aside className="lg:w-[380px] flex-shrink-0">
            <div className="lg:sticky lg:top-24 mt-32">
              {/* Categories */}
              <div className="bg-white rounded-lg shadow-md border border-gray-100 p-4 mb-6">
                <h2 className="text-xl font-bold text-gray-800 mb-4 text-left">
                  Categories
                </h2>
                <div className="w-16 h-1 bg-[#FF6E04] mb-6" />
                <div className="space-y-1">
                  {blogCategories?.map((cat) => (
                    <Link
                      key={cat.id}
                      href={`/blogs/category/${cat.slug}`}
                      className="flex justify-between items-center p-2 hover:bg-gray-50 rounded transition-colors border-l-2 border-transparent hover:border-[#FF6E04]"
                    >
                      <span className="text-gray-700 text-sm font-medium">
                        {cat.name}
                      </span>
                      <span className="bg-gray-100 text-gray-600 px-3 py-1 rounded-full text-sm">
                        {cat.counts}
                      </span>
                    </Link>
                  ))}
                </div>
              </div>

              {/* Popular Posts */}
              <div className="bg-white rounded-lg shadow-md border border-gray-100 p-5 mb-6">
                <h2 className="text-xl font-bold text-gray-800 mb-4 text-left">
                  Popular Posts
                </h2>
                <div className="w-16 h-1 bg-[#FF6E04] mb-6" />
                <div className="space-y-4">
                  {mostViewedBlogs?.map((blog) => (
                    <Link
                      key={blog.id}
                      href={`/blogs/${blog.slug}`}
                      className="block pb-4 border-b border-gray-100 last:border-b-0 hover:translate-x-1 transition-transform"
                    >
                      <h3 className="text-gray-800 text-sm font-semibold mb-1 hover:text-[#FF6E04] transition-colors text-left">
                        {blog.title}
                      </h3>
                    </Link>
                  ))}
                </div>
              </div>

              {/* Ad banners */}
              <div className="bg-gradient-to-br from-orange-50 to-orange-100 rounded-lg shadow-md border-2 border-[#FF6E04] p-8 mb-6 text-center">
                <h4 className="text-xl font-bold text-gray-800 mb-2">
                  Advertise Here
                </h4>
                <p className="text-gray-600">300 × 250</p>
              </div>
              <div className="bg-gradient-to-br from-gray-50 to-gray-100 rounded-lg shadow-md border border-gray-200 p-8 text-center">
                <h4 className="text-xl font-bold text-gray-800 mb-2">
                  Sponsored Content
                </h4>
                <p className="text-gray-600 mb-4">300 × 250</p>
                <Link
                  href="/contact-us"
                  className="inline-block text-[#FF6E04] border border-[#FF6E04] px-6 py-2 rounded hover:bg-[#FF6E04] hover:text-white transition-all duration-300 font-medium"
                >
                  Contact Us
                </Link>
              </div>
            </div>
          </aside>
        </div>

        {/* Blog content styles — unchanged from original */}
        <style jsx global>{`
          /* … paste your existing .blog-content styles here … */
        `}</style>
      </div>
    </>
  );
}
