...
// ── STATUS CONFIG ─────────────────────────────────────────────────────────────
const STATUS_CONFIG = {
  captured:   { label: "Paid",        bg: "bg-emerald-50", text: "text-emerald-700", dot: "bg-emerald-500", border: "border-emerald-200" },
  created:    { label: "Pending",     bg: "bg-amber-50",   text: "text-amber-700",   dot: "bg-amber-400",   border: "border-amber-200" },
  ...
  authorized: { label: "Authorized",  bg: "bg-blue-50",    text: "text-blue-700",    dot: "bg-blue-500",    border: "border-blue-200" },
};
...
export function InvoiceModal({ payment, onClose }) {
  ...
  return (
    ...
    >
      ...
      >
        {/* Receipt top tear */}
        <div className="w-full h-4 relative overflow-hidden" style={{ background: "#1a1a2e" }}>
          <div className="absolute bottom-0 left-0 right-0 flex">
            {Array.from({ length: 28 }).map((_, i) => (
              <div key={i} className="flex-1 h-2 rounded-t-full" style={{ background: "#f8f7f4", margin: "0 1px" }} />
            ))}
          </div>
        </div>
        ...
        <div style={{ background: "#f8f7f4" }} className="px-8 pb-0 pt-6">
          ...
          <div className="text-center mb-6">
            <div className="text-[11px] tracking-[0.3em] text-slate-400 uppercase mb-1">Payment Receipt</div>
            <div className="text-[22px] font-black tracking-tight text-slate-900" >
              AddressGuru
            </div>
            <div className="text-[10px] text-slate-400 mt-1 tracking-widest">addressguru.ae</div>
          </div>
          ...
          <div className="flex justify-between text-[11px] text-slate-500 mb-5">
            <div>
              <div className="text-[9px] uppercase tracking-widest text-slate-400 mb-0.5">Receipt No.</div>
              <div className="font-bold text-slate-700">{payment.receipt}</div>
            </div>
            <div className="text-right">
              <div className="text-[9px] uppercase tracking-widest text-slate-400 mb-0.5">Date</div>
              <div className="font-bold text-slate-700">{fmtDate(payment.createdAt)}</div>
          ...
          {/* Amount — big */}
          <div className="text-center mb-6">
            <div className="text-[11px] uppercase tracking-widest text-slate-400 mb-1">Amount Charged</div>
          ...
          <div className="space-y-3 mb-5">
            {[
              ...
              ...(payment.razorpay?.paymentId ? [{ label: "Payment ID", value: payment.razorpay.paymentId }] : []),
              ...(payment.listing?.businessName ? [{ label: "Listing", value: payment.listing.businessName }] : []),
            ].map(({ label, value }) => (
              <div key={label} className="flex justify-between items-start gap-3">
                <span className="text-[11px] uppercase tracking-wider text-slate-400 whitespace-nowrap">{label}</span>
                <span className="text-[11px] font-bold text-slate-700 text-right break-all max-w-[200px]">{value}</span>
              </div>
            ))}
          </div>
          ...
          {snap.features?.length > 0 && (
            <>
              ...
              <div className="mb-5">
                <div className="text-[9px] uppercase tracking-widest text-slate-400 mb-2">Included Features</div>
        ...
        {/* Receipt bottom tear */}
        <div className="w-full h-4 relative overflow-hidden" style={{ background: "#f8f7f4" }}>
          <div className="absolute top-0 left-0 right-0 flex">
            {Array.from({ length: 28 }).map((_, i) => (
              <div key={i} className="flex-1 h-2 rounded-b-full" style={{ background: "#e5e3dc", margin: "0 1px" }} />
            ))}
          </div>
        </div>
      </div>
...
export function BillCard({ payment, index, page, showEntries, onView }) {
  ...
  return (
    ...
    >
      ...
      {/* Icon */}
      <div className={`w-10 h-10 rounded-xl flex-shrink-0 flex items-center justify-center ${cfg.bg} border ${cfg.border}`}>
        <svg className={`w-5 h-5 ${cfg.text}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="text-[11px] text-slate-400 mt-0.5 flex items-center gap-2">
          ...
          {payment.listing?.businessName && (
            <>
              <span>·</span>
              <span className="text-blue-500 truncate max-w-[120px]">{payment.listing.businessName}</span>
            </>
          )}
        </div>
      </div>
      ...
      <div className="flex-shrink-0 text-right">
        <div className="text-[15px] font-black text-slate-900">{fmtAmount(payment.amount, payment.currency)}</div>
...
// ── REUSABLE: PAGINATION ──────────────────────────────────────────────────────
export function Pagination({ page, totalPages, totalCount, showEntries, setPage }) {
  ...
  return (
    <div className="flex items-center justify-between mt-4 px-1">
      <span className="text-xs text-slate-500 font-medium">
        Showing <span className="font-bold text-slate-700">{totalCount === 0 ? 0 : startEntry}</span> to{" "}
  ...
  return (
    ...
    >
      ...
      <div className="mb-7">
        ...
        <p className="text-sm text-slate-500 mt-0.5">Your invoices and transaction history</p>
      </div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-7">
        {[
          ...
          <div key={c.label} className={`${c.bg} border border-slate-200 rounded-xl p-4 flex items-center gap-3`}>
            ...
            <div>
              <div className="text-[11px] text-slate-400 font-medium uppercase tracking-wide">{c.label}</div>
              <div className="text-[20px] font-black text-slate-800 leading-tight">{c.value}</div>
      ...
      <div className="mb-3 flex items-center justify-between">
        <h2 className="text-[13px] font-bold text-slate-600 uppercase tracking-widest">Recent Invoices</h2>
      ...
      {loading && (
        <div className="flex flex-col items-center py-16 gap-2 text-slate-400">
          <svg className="w-5 h-5 animate-spin text-blue-500" viewBox="0 0 24 24" fill="none">
            ...
            <path d="M22 12a10 10 0 00-10-10" stroke="currentColor" strokeWidth="3" className="opacity-75" />
          </svg>
      ...
      {!loading && payments.length === 0 && (
        <div className="flex flex-col items-center py-20 gap-3 text-slate-300">
          ...
          <span className="text-sm font-medium text-slate-400">No invoices yet</span>
          <span className="text-xs text-slate-400">Your payment history will appear here</span>
        </div>
      )}
