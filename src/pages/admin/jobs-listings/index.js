...
import {
  approve_jobs_listing,
  approve_reject_jobs_listing,
...
const JobsListings = () => {
  ...
  async function handleApproveReject(listing, action) {
    try {
      ...
      if (action === "approved") {
        await approve_jobs_listing(listing._id, {
          status: "approved",
        });
      }
      ...
      showToast(`Job listing ${action} successfully`, "success");
      await fetchListings();
    } catch (error) {
  ...
  async function handleRejectSubmit({ listingId, reason }) {
    try {
      await approve_reject_jobs_listing(listingId, {
        status: "rejected",
        rejectionReason: reason,
      });
      ...
      showToast("Job listing rejected successfully", "success");
    } catch (error) {
      showToast("Failed to reject job listing", "error");
    }
  }
  ...
  const fetchListings = async () => {
    try {
      ...
      setListings(res?.data?.jobs || res?.data?.listings || []);
      setTotalPages(res?.data?.pagination?.totalPages || 1);
      setTotalCount(res?.data?.pagination?.total || 0);

      setTotalJobs(res?.data?.totalAll || 0);
      setApprovedJobs(res?.data?.statusCounts?.approved || 0);
      setRejectedJobs(res?.data?.statusCounts?.rejected || 0);
      setPendingJobs(res?.data?.statusCounts?.pending || 0);
    } catch (err) {
      console.error(err);
    }
  };
  ...
  const statCounts = {
    pending: statusFilter === "pending" ? totalCount : pendingJobs,
    approved: statusFilter === "approved" ? totalCount : approvedJobs,
    rejected: statusFilter === "rejected" ? totalCount : rejectedJobs,
  };
          ...
          <table className="w-full border-collapse" style={{ minWidth: 1100 }}>
            <thead>
              <tr className="bg-slate-50 border-b-2 border-slate-200">
                ...
                <TH_Cell className="w-28">Job Type</TH_Cell>
                  ...
                  >
                        ...
                        <div className="min-w-0 flex-1 pt-0.5">
                          ...
                          <div className="flex items-center gap-1.5 mt-1 flex-wrap">
                            ...
                            <Link
                              ...
                              href={`/dashboard/jobs-listing?jobId=${listing?.slug}&edit=true`}
                            >
                              Edit
                            </Link>
                          </div>
                    ...
                    <TD vAlign="top" className="w-28">
                      <div className="mt-0.5">
                        {listing?.jobType ? (
                          <span className="inline-flex items-center px-2 py-0.5 rounded-md text-[10px] font-bold text-emerald-600 bg-emerald-100 capitalize">
                            {listing.jobType}
                          </span>
