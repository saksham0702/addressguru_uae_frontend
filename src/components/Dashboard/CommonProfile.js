...
import { APP_URL, COUNTRY_CODES } from "@/services/constants";
...
const CommonProfile = () => {
  ...
  const handleInputChange = (e) => {
    ...
    // Feature Request: Max 10 digits for phone number
    if (name === "phone") {
    ...
    <div className="w-full">
      ...
      <div className="bg-white rounded-xl overflow-hidden">
        <div className="px-8 pb-8">
          ...
          <div className="grid md:grid-cols-2 gap-6 mb-8">
              ...
              {isEditing ? (
                <div className="flex gap-2 relative">
                  <div className="relative w-36" ref={countryDropdownRef}>
                    ...
                    {showCountryDropdown && (
                      <div className="absolute z-50 mt-1 w-64 bg-white border border-gray-200 rounded-lg shadow-lg max-h-60 overflow-y-auto text-black">
                        <input
                          type="text"
            ...
            <div>
              ...
              {isEditing ? (
                ...
                <p className={`font-medium px-4 py-2.5 ${userData?.dob ? "text-gray-900" : "text-gray-400"}`}>
                  {userData?.dob ? new Date(userData.dob).toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" }) : "Not Updated"}
                </p>
              )}
            </div>
          ...
          {/* Hobbies */}
          <div className="mb-8">
            <label className="block text-sm font-medium text-gray-700 mb-3">Hobbies & Interests</label>
            <div className="flex flex-wrap gap-2 mb-3">
              {Array.isArray(formData?.hobbies) &&
                formData.hobbies.map((hobby, index) => (
                  <span key={index} className="inline-flex items-center gap-2 px-4 py-2 bg-blue-100 text-blue-700 rounded-full text-sm font-medium">
                    {hobby}
                    {isEditing && (
                      <button onClick={() => removeHobby(index)} className="hover:text-blue-900">
                        <X size={14} />
                      </button>
                    )}
                  </span>
            ...
            {isEditing && (
              <div className="flex gap-2">
                ...
                />
                <button onClick={addHobby} className="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors">Add</button>
              </div>
            )}
          </div>
          ...
          <div className="mb-8">
            <label className="block text-sm font-medium text-gray-700 mb-3">
              <LinkIcon size={16} className="inline mr-2" /> Social Links
            </label>
            <div className="grid md:grid-cols-2 gap-4">
              {["facebook", "linkedin", "instagram", "telegram"].map((platform) => (
                <div key={platform}>
                  <label className="block text-xs text-gray-600 mb-1 capitalize">{platform}</label>
                  {isEditing ? (
                    <input
                      type={platform === "telegram" ? "text" : "url"}
                      value={formData?.socialLinks?.[platform] || ""}
                      onChange={(e) => handleSocialLinkChange(platform, e.target.value)}
                      placeholder={platform === "telegram" ? "@username" : `https://${platform}.com/username`}
                      className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm"
                    />
                  ) : userData?.socialLinks?.[platform] ? (
                    platform === "telegram" ? (
                      <p className="text-gray-900 text-sm px-4 py-2">{userData.socialLinks[platform]}</p>
                    ) : (
                      <a href={userData.socialLinks[platform]} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline text-sm block px-4 py-2 break-all">
                        {userData.socialLinks[platform]}
                      </a>
                    )
                  ) : (
                    <p className="text-gray-400 text-sm px-4 py-2">Not provided</p>
                  )}
                </div>
              ))}
            </div>
          ...
          {isEditing && (
            <div className="border-t pt-8">
              ...
              <div className="grid md:grid-cols-3 gap-4">
                {[
                  { label: "Current Password", name: "oldPassword", show: showPassword, setShow: setShowPassword },
                  ...
                  <div key={field.name}>
                    ...
                    <div className="relative">
                      ...
                      />
                      <button type="button" onClick={() => field.setShow(!field.show)} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-500 hover:text-gray-700">
