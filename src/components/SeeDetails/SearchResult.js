...
const SearchResults = ({
  ...
  useEffect(() => {
    ...
    const formatted =
      ssrCity.charAt(0).toUpperCase() + ssrCity.slice(1);
    if (cityName.toLowerCase() !== ssrCity.toLowerCase()) {
      setCity(formatted);
    }
  }, [ssrCity]);
  ...
  const applyLocalFilters = (data, f) => {
    ...
    if (f.ag_verified) result = result.filter((item) => item.isVerified === true);
    if (f.facilities_id?.length > 0)
      result = result.filter((item) => matchIds(item.facilities, f.facilities_id));
    ...
    if (f.payment_mode_id?.length > 0)
      result = result.filter((item) => matchIds(item.paymentModes, f.payment_mode_id));
    if (f.sort_by === "newest")
  ...
  // ── Fetch dynamic filter options (facilities, services etc) ──
  useEffect(() => {
    ...
    const fetchCategoryFeatures = async () => {
      try {
        ...
        setDynamicFilters({
          facilities: data?.features?.facilities?.map((f) => ({ id: f._id, name: f.name })) || [],
          services: data?.features?.services?.map((s) => ({ id: s._id, name: s.name })) || [],
          courses: data?.features?.courses?.map((c) => ({ id: c._id, name: c.name })) || [],
          paymentModes: data?.payment_modes?.map((p) => ({ id: p._id, name: p.name })) || [],
        });
      } catch (err) {
  ...
  // ── Fetch listings (skips first mount since SSR handled it) ──
  useEffect(() => {
    if (!router.isReady || !slug || !cityName) return;

    // Skip first mount — SSR already gave us the data
  ...
  const handleLoadMore = async () => {
    ...
    const citySlug = cityName.toLowerCase().replace(/\(.*\)/, "").replace(/\s+/g, "-");
    try {
      ...
      <div className="h-auto flex flex-col max-md:mt-1.5 items-center overflow-hidden justify-center bg-[#F8F7F7]">
        <div className="flex flex-col min-md:w-[80%] max-md:min-w-full bg-white md:px-3 mx-auto md:pb-20 pr-2">
          ...
          <h1 className="font-bold text-xl mt-2 capitalize max-md:hidden mb-3">
            Top {apiListings?.[0]?.category?.name || canonicalSlug} in {canonicalCity}
          </h1>
                  ...
                  <div className="flex justify-center items-center py-12 px-4">
                    <div className="flex flex-col items-center text-center max-w-md w-full">
                      ...
                      <div className="flex items-center gap-1.5 bg-orange-50 border border-orange-200 rounded-full px-3 py-1 mb-4">
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-3.5 w-3.5 text-orange-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                          <path strokeLinecap="round" strokeLinejoin="round" d="M15 10.5a3 3 0 11-6 0 3 3 0 016 0z" />
                          <path strokeLinecap="round" strokeLinejoin="round" d="M19.5 10.5c0 7.142-7.5 11.25-7.5 11.25S4.5 17.642 4.5 10.5a7.5 7.5 0 1115 0z" />
                        </svg>
                        <span className="text-xs font-medium text-orange-600">{globalCity}</span>
                      </div>
                      <h2 className="text-xl font-semibold text-gray-800 mb-2">Be the first to list your business here!</h2>
                      <p className="text-sm text-gray-500 mb-1 leading-relaxed">
                        No listings yet in <span className="font-medium text-gray-700">{globalCity}</span> under <span className="font-medium text-gray-700">{slug}</span>.
                      </p>
                      <p className="text-sm text-gray-500 mb-7 leading-relaxed">
                        Get ahead of the competition — add your business and start reaching local customers today.
                      </p>
                      ...
                      <p className="text-xs text-gray-400 mt-5">
                        Free to list &nbsp;·&nbsp; Reach local buyers &nbsp;·&nbsp; Takes under 2 minutes
                      </p>
                    </div>
                  </div>
                ) : (
